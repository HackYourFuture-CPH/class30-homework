...
FROM `task`; /*all tasks 35*/

SELECT COUNT(*) 
FROM `task` 
WHERE due_date IS NULL; /*tasks with null due_date 8*/
SELECT COUNT(*) 
FROM `task` as tk 
JOIN `status` as st ON tk.status_id = st.id
WHERE st.name = "Done"; /* all tasks with status Done 12*/

SELECT COUNT(*) 
FROM `task` as tk 
JOIN `status` as st ON tk.status_id = st.id
WHERE st.name != "Done"; /* all tasks that are not done 23*/
